...
function Header() {
  ...
  return (
    <header className="glass-chrome depth-surface sticky top-0 z-40 flex shrink-0 items-center gap-3 border-b px-4 py-3 text-[hsl(var(--foreground))]">
      <button type="button" data-testid="button-close-app" aria-label="ዝጋ" onClick={close} className="rounded-xl p-1.5 transition-transform active:scale-90 hover:bg-[hsl(185_100%_36%/.12)]">
        <X className="h-6 w-6" />
      </button>
      <button type="button" data-testid="button-brand-home" onClick={() => setLocation('/')} className="flex flex-1 items-center gap-2 text-left">
        <span className="text-shimmer text-[22px] font-extrabold tracking-tight">last bingo</span>
      </button>
      <button type="button" data-testid="button-header-dropdown" aria-label="አማራጮች" onClick={() => setMenuOpen((open) => !open)} className="rounded-xl p-1.5 transition-transform active:scale-90 hover:bg-[hsl(185_100%_36%/.12)]">
        <ChevronDown className="h-7 w-7" />
      </button>
      <button type="button" data-testid="button-header-menu" aria-label="ምናሌ" onClick={() => setMenuOpen((open) => !open)} className="rounded-xl p-1.5 transition-transform active:scale-90 hover:bg-[hsl(185_100%_36%/.12)]">
        <MoreVertical className="h-6 w-6" />
      </button>
      {menuOpen && (
        <div className="absolute right-3 top-14 z-30 w-48 overflow-hidden rounded-2xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] p-1.5 shadow-2xl animate-rise-in">
          <button type="button" data-testid="button-header-agent" onClick={() => { setMenuOpen(false); setLocation('/agent'); }} className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-sm text-[hsl(var(--primary))] hover:bg-[hsl(185_100%_36%/.1)]">
            <Gift className="h-4 w-4" /> የወኪል ፕሮግራም
          </button>
          {isAdmin && <button type="button" data-testid="button-header-admin" onClick={() => { setMenuOpen(false); setLocation('/admin'); }} className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-sm text-[hsl(var(--primary))] hover:bg-[hsl(185_100%_36%/.1)]">
            <ShieldCheck className="h-4 w-4" /> አድሚን ፓናል
          </button>}
          <button type="button" data-testid="button-header-help" onClick={() => setMenuOpen(false)} className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-sm hover:bg-[hsl(185_100%_36%/.1)]">
...
function BottomNav({ active, onChange }: { active: Tab; onChange: (tab: Tab) => void }) {
  return (
    <nav className="glass-chrome bottom-nav depth-surface sticky bottom-0 z-40 grid shrink-0 grid-cols-3 border-t px-3 pb-[max(10px,env(safe-area-inset-bottom))] pt-2">
      {(['bingo', 'leaderboard', 'wallet'] as Tab[]).map((tab) => {
        ...
        return (
          <button
            ...
            className={`bottom-nav-button relative flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-2xl text-[11px] font-bold tracking-[.12em] transition-all duration-200 active:scale-95 ${selected ? 'bottom-nav-selected text-[hsl(var(--primary))]' : 'text-[hsl(var(--muted-foreground))]'}`}
          >
...
function Stats({ play, pot, cardsTaken, stake }: { play: number; pot: number; cardsTaken: number; stake: number }) {
  const money = (amount: number) => amount.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 });
  return (
    <div className="balance-stats grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,.72fr)] gap-2 px-3 pt-3">
      <div className="balance-stat balance-stat-play selection-summary-card depth-card rounded-2xl border px-2.5 py-2.5">
        <div className="balance-stat-heading"><span className="balance-stat-label">MAIN WALLET</span></div>
        <div data-testid="text-play-stake" className="balance-stat-amount mt-1 font-mono text-lg font-extrabold">{money(play)}</div>
      </div>
      <div className="balance-stat balance-stat-prize selection-summary-card depth-card relative overflow-hidden rounded-2xl border px-2.5 py-2.5 text-center">
        <span className="absolute -right-5 -top-7 h-16 w-16 rounded-full bg-[hsl(var(--selection-accent)/.12)]" />
        <div className="balance-stat-heading relative justify-center"><span className="balance-stat-label">PLAY WALLET</span></div>
        <div data-testid="text-pot" className="balance-stat-amount relative mt-1 font-mono text-lg font-extrabold">{money(pot)}</div>
        <div data-testid="text-cards-taken" className="balance-stat-cards relative mt-0.5 text-[9px]">{cardsTaken} cards</div>
      </div>
      <div className="balance-stat balance-stat-win selection-summary-card depth-card rounded-2xl border px-2.5 py-2.5 text-center">
        <div className="balance-stat-heading justify-center"><span className="balance-stat-label">STAKE</span></div>
        <div data-testid="text-win" className="balance-stat-amount mt-1 font-mono text-lg font-extrabold">{money(stake)}</div>
        <div className="balance-stat-unit text-[10px]">s</div>
      </div>
    </div>
  );
}

function SoundCountdown({ countdown, status }: { countdown: number; status: string }) {
  ...
  return (
    <section className="px-3 pt-4">
      ...
      <div className="mt-3 h-px bg-[hsl(var(--accent)/.1)]" />
    </section>
  );
}
...
function MiniCard({ id, grid }: { id: number; grid: Cell[] }) {
  return (
    <div className="depth-card w-[76px] shrink-0 rounded-xl border border-[hsl(var(--primary)/.55)] bg-[hsl(var(--card)/.96)] p-1.5 shadow-lg">
      ...
      <div className="grid grid-cols-5 gap-0.5">{grid.map((cell, index) => <div key={`${id}-${index}`} className={`grid aspect-square place-items-center rounded-[3px] text-[8px] font-bold ${cell === 'star' ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : 'bg-[hsl(262_45%_18%)] text-[hsl(262_25%_88%)]'}`}>{cell === 'star' ? '✦' : cell}</div>)}</div>
    </div>
  );
}
...
function NumberGrid({ selected, taken, onToggle, canSelect, maxCards }: { selected: Set<number>; taken: Set<number>; onToggle: (number: number) => void; canSelect: boolean; maxCards: number }) {
  return (
    <div className="selection-number-panel px-3 pb-28 pt-4">
      <div className="mb-3 flex items-end justify-between">
        <div><h2 className="text-lg font-extrabold">ካርድ ይምረጡ</h2><p className="mt-0.5 text-xs text-[hsl(var(--selection-muted))]">እስከ {maxCards} ካርዶች · አንድ ካርድ {STAKE} ብር</p></div>
        <div className="selection-count rounded-full px-2.5 py-1 text-[11px] font-bold">{selected.size}/{maxCards}</div>
      </div>
      <div className="selection-number-grid grid grid-cols-8 gap-1.5">
        {Array.from({ length: TOTAL_NUMBERS }, (_, index) => index + 1).map((number) => {
          ...
          return (
            <button type="button" key={number} data-testid={`button-card-${number}`} disabled={!canSelect || (isTaken && !isSelected)} onClick={() => onToggle(number)} className={`selection-number depth-action relative aspect-square rounded-xl border text-xs font-bold transition-all duration-150 active:scale-90 ${isSelected ? 'selection-number-selected -translate-y-0.5' : isTaken ? 'selection-number-taken' : 'selection-number-available'}`}>
              {number}
...
function WalletPanel() {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-3 pb-5 pt-5">
      <section className="depth-surface relative rounded-[32px] bg-[hsl(var(--card))] p-6 shadow-[0_12px_35px_hsl(222_26%_18%/.08)]">
        ...
        <div className="mt-5 grid grid-cols-2 gap-5">
          <div className="depth-card rounded-[26px] border border-[hsl(var(--foreground)/.8)] px-4 py-4">
            <div className="wallet-balance-label"><span aria-hidden="true">🎮</span><span>PLAY BALANCE</span></div>
            <div data-testid="text-play-wallet-balance" className="mt-3 font-mono text-2xl font-bold">{playWallet}</div>
          </div>
          <div className="depth-card rounded-[26px] border border-[hsl(var(--foreground)/.8)] px-4 py-4">
            <div className="wallet-balance-label"><span aria-hidden="true">🏆</span><span>WINNINGS</span></div>
      ...
      <section className="depth-surface relative mt-7 min-h-28 rounded-[28px] bg-[hsl(var(--card))] p-6">
...
function LeaderboardPanel() {
  ...
  return <div className="flex-1 overflow-y-auto px-3 pb-5 pt-5">
    <section className="depth-surface rounded-[28px] border border-[hsl(var(--primary)/.35)] bg-[hsl(var(--card))] p-5 shadow-[0_12px_35px_hsl(222_26%_18%/.08)]">
      <div className="flex items-start justify-between gap-3"><div><h2 className="text-shimmer mt-1 text-2xl font-extrabold">LEADERBOARD</h2></div><Trophy className="h-8 w-8 text-[hsl(var(--primary))]" /></div>
    ...
    <section className="mt-5"><div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-extrabold tracking-[.1em]">TOP PLAYERS</h3><span className="text-[10px] text-[hsl(var(--muted-foreground))]">50% · 30% · 20%</span></div><div className="space-y-2">{data?.entries.length ? data.entries.map((entry) => <article key={entry.telegramId} className={`depth-card flex items-center gap-3 rounded-2xl border bg-[hsl(var(--card))] px-3 py-3 ${entry.telegramId === data.me?.telegramId ? 'border-[hsl(var(--primary)/.8)]' : 'border-[hsl(198_60%_74%)]'}`}><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl font-mono text-sm font-bold ${entry.rank <= 3 ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : 'bg-[hsl(262_45%_18%)] text-[hsl(var(--muted-foreground))]'}`}>{entry.rank}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{entry.name || 'Player'}</p><p className="text-[10px] text-[hsl(var(--muted-foreground))]">{entry.rank <= 3 ? `🏆 ${entry.prize} ብር` : 'ተሳታፊ'}</p></div><strong className="font-mono text-lg text-[hsl(var(--primary))]">{entry.score}</strong></article>) : <div className="depth-surface rounded-2xl p-5 text-center text-xs text-[hsl(var(--muted-foreground))]">እስካሁን ነጥብ ያለው ተጫዋች የለም።</div>}</div></section>
  </div>;
}
  ...
  return (
    <AppShell tab={tab} setTab={setTab}>
      {tab === 'wallet' ? <WalletPanel /> : tab === 'leaderboard' ? <LeaderboardPanel /> : <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        {selectedCards.length > 0 && <div className="absolute bottom-[74px] left-0 right-0 z-10 flex gap-2 overflow-x-auto overflow-y-hidden bg-gradient-to-t from-[hsl(var(--background))] to-transparent px-3 pb-2 pt-8">{selectedCards.map((id) => <MiniCard key={id} id={id} grid={buildCard(id)} />)}</div>}
        {showWarning && <div role="alert" data-testid="status-card-limit" className="absolute left-4 right-4 top-24 z-30 rounded-2xl border border-[hsl(var(--primary)/.6)] bg-[hsl(var(--card)/.98)] px-4 py-3 text-center text-sm font-bold text-[hsl(var(--primary))] shadow-xl animate-rise-in">{warningMessage || `ከ${maxCards} ካርድ በላይ መምረጥ አይችሉም`}</div>}
      </div>}
    </AppShell>
  );
}
...
function CalledBoard({ called, latest }: { called: Set<number>; latest: number | null }) {
  ...
  return <section className="depth-surface relative rounded-2xl border border-[hsl(var(--accent)/.25)] bg-[hsl(var(--card))] px-2 py-2 shadow-[0_0_18px_hsl(var(--accent)/.06)]"><div className="space-y-1">{columns.map((letter, row) => <div key={letter} className="grid grid-cols-[2rem_repeat(15,minmax(0,1fr))] items-center gap-1.5"><span className="text-lg font-extrabold leading-none text-[hsl(var(--foreground))]">{letter}</span>{Array.from({ length: 15 }, (_, index) => { const number = row * 15 + index + 1; return <span key={number} data-testid={`status-called-${number}`} className={`grid aspect-square place-items-center rounded-full font-mono text-[8px] font-bold transition-all ${latest === number ? 'called-number bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : called.has(number) ? 'called-number bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : 'bg-[hsl(263_45%_18%)] text-[hsl(var(--muted-foreground)/.65)]'}`}>{number}</span>; })}</div>)}</div></section>;
}
...
function CalledPanel({ current, muted, onToggle, callIndex, called }: { current: number | null; muted: boolean; onToggle: () => void; callIndex: number; called: Set<number> }) {
  ...
  return <section className="depth-surface live-call-panel relative flex items-center gap-3 rounded-2xl border border-[hsl(var(--accent)/.35)] bg-[hsl(var(--card))] p-3 shadow-[0_8px_30px_hsl(var(--accent)/.08)]"><div key={current ?? 'waiting'} className="live-call-orb spin-reveal-ball relative grid h-[76px] w-[76px] shrink-0 place-items-center rounded-full font-mono text-3xl font-bold text-[hsl(var(--primary-foreground))]"><span className="absolute -inset-1 rounded-full border border-[hsl(var(--primary)/.35)] animate-pulse-ring" /><span className="absolute top-2 text-xs font-extrabold">{currentLetter}</span><span className="call-ball-value mt-3">{current ?? '—'}</span></div><div className="min-w-0 flex-1"><div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[.16em] text-[hsl(var(--muted-foreground))]"><Timer className="h-3.5 w-3.5 text-[hsl(var(--primary))]" /> LIVE CALL</div><h2 className="mt-1 truncate text-lg font-extrabold"><strong className="live-call-current-number">{current ?? '—'}</strong></h2><div className="mt-2 flex flex-row gap-1.5 [direction:ltr]">{recentCalls.map((number) => <span key={number} className="rounded-full bg-[hsl(var(--primary))] px-2.5 py-1 font-mono text-xs font-bold text-[hsl(var(--primary-foreground))]">{number}</span>)}</div></div><div className="flex shrink-0 flex-col items-center gap-1"><button type="button" data-testid="button-play-mute" onClick={onToggle} aria-label={muted ? 'ድምፅ አብራ' : 'ድምፅ ዝጋ'} className={`grid h-10 w-10 place-items-center rounded-xl border transition-colors ${muted ? 'border-[hsl(var(--destructive)/.7)] text-[hsl(var(--destructive))]' : 'border-[hsl(var(--accent)/.7)] text-[hsl(var(--accent))]'}`}>{muted ? <VolumeX className="h-5 w-5" /> : <Volume2 className="h-5 w-5" />}</button><span data-testid="text-called-count" className="font-mono text-xl font-bold">{called.size}/75</span></div></section>;
}
...
function playWinnerAnnouncement() {
  ...
  [523.25, 659.25, 783.99].forEach((frequency, index) => {
    ...
    gain.gain.exponentialRampToValueAtTime(0.22, now + index * 0.16 + 0.03);
...
function GamePage() {
  ...
  useEffect(() => {
    ...
    if (!muted) playWinnerAnnouncement();
  ...
  return <AppShell tab={tab} setTab={setTab} gameMode>{tab === 'wallet' ? <WalletPanel /> : tab === 'leaderboard' ? <LeaderboardPanel /> : <div className="selection-screen min-h-0 flex-1 overflow-y-auto p-3"><div className="space-y-3"><div className="flex items-center justify-between gap-3 text-[10px] font-bold tracking-[.12em] text-[hsl(var(--muted-foreground))]"><div className="flex min-w-0 items-center gap-2"><span className="shrink-0">{connected ? '● LIVE' : '○ CONNECTING...'}</span><div className="game-prize-pill flex min-w-0 items-center gap-1.5 rounded-full border border-[hsl(var(--primary)/.35)] bg-[hsl(var(--primary)/.08)] px-2.5 py-1 text-[9px] tracking-[.08em] text-[hsl(var(--foreground)/.82)]"><span className="game-prize-pill-label">PRIZE POOL</span><strong data-testid="text-game-prize-pool" className="font-mono text-[11px] font-extrabold tracking-normal text-[hsl(var(--primary))]">{gameState.netPrizePool.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ብር</strong><span className="text-[hsl(var(--primary)/.55)]">•</span><span>ካርዶች</span><strong data-testid="text-game-cards-taken" className="font-mono text-[11px] font-extrabold tracking-normal text-[hsl(var(--accent))]">{gameState.playersWithCards}</strong></div></div><div className="flex shrink-0 items-center gap-1.5"><button type="button" data-testid="button-toggle-game-debug" aria-expanded={false} onClick={() => undefined} className="hidden">DEBUG</button><button type="button" data-testid="button-refresh-game" aria-label="Refresh game" onClick={refreshGame} className="depth-action inline-flex items-center gap-1.5 rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-2 text-[hsl(var(--foreground)/.8)] transition-transform active:scale-95"><RotateCcw className="h-3.5 w-3.5" /> REFRESH</button></div></div>{null}<CalledBoard called={called} latest={current} /><CalledPanel current={current} muted={muted} onToggle={() => setMuted((value) => !value)} callIndex={gameState.calledBalls.length} called={called} /><div className="grid grid-cols-2 gap-2.5">{displayCards.map((card) => { const pattern = winnerMatch?.card.id === card.id ? winnerMatch.pattern : null; return <PlayCard key={card.id} {...card} called={called} winner={pattern} />; })}</div><div className="flex items-center justify-center gap-2 pb-2 text-[11px] text-[hsl(var(--muted-foreground))]"><Sparkles className="h-3.5 w-3.5 text-[hsl(var(--primary))]" /> ቁጥሮች በየ 3 ሰከንዱ ይጠራሉ</div></div></div>}{visibleWinnerSnapshot && visibleWinnerSnapshot.winners.length > 1 ? <MultipleWinnerModal winners={visibleWinnerSnapshot.winners} finalNumber={Array.from(visibleWinnerSnapshot.called).at(-1) ?? null} onContinue={() => { sessionStorage.removeItem('selectedSlots'); setLocation('/'); }} /> : visibleWinnerSnapshot ? <WinnerModal card={visibleWinnerSnapshot.card} called={visibleWinnerSnapshot.called} pattern={visibleWinnerSnapshot.pattern} winnerEffect={visibleWinnerSnapshot.winnerEffect} prize={visibleWinnerSnapshot.winner.payout} winnerName={visibleWinnerSnapshot.winner.name ?? ''} winnerCount={visibleWinnerSnapshot.winners.length} /> : showServerWinner && serverWinners.length > 1 ? <MultipleWinnerModal winners={serverWinners} finalNumber={Array.from(called).at(-1) ?? null} onContinue={() => { sessionStorage.removeItem('selectedSlots'); setLocation('/'); }} /> : showServerWinner && serverWinner && winnerCard && winnerPattern ? <WinnerModal card={winnerCard} called={called} pattern={winnerPattern} winnerEffect={winnerEffect} prize={serverWinner.payout} winnerName={serverWinner.name ?? ''} winnerCount={serverWinners.length} /> : showServerWinner && serverWinner && !gameOverError ? <RoundFinishedModal winnerName={serverWinner.name ?? ''} cardNumber={serverWinner.cardNumber} prize={serverWinner.payout} onContinue={() => { sessionStorage.removeItem('selectedSlots'); setLocation('/'); }} /> : gameOverError ? <GameOverError message={gameOverError} onRetry={refreshGame} /> : null}</AppShell>;
}

type AgentProgram = {
  agent: { id: number; code: string; status: 'pending' | 'active' | 'suspended'; commissionRate: string } | null;
  referralLink?: string | null;
  stats?: { referrals: number; depositTotal: string; commissionTotal: string; agentWalletBalance: string };
  commissions?: Array<{ id: number; depositAmount: string; commissionAmount: string; commissionRate: string; createdAt: string }>;
  withdrawals?: Array<{ id: number; amount: string; phone: string; ownerName: string; status: string; createdAt: string }>;
};

function AgentDashboard() {
  const [, setLocation] = useLocation();
  const [program, setProgram] = useState<AgentProgram | null>(null);
  const [state, setState] = useState<'loading' | 'ready' | 'auth-required' | 'error'>('loading');
  const [error, setError] = useState('');
  const [applying, setApplying] = useState(false);
  const [copied, setCopied] = useState(false);
  const [withdrawalAmount, setWithdrawalAmount] = useState('');
  const [withdrawalPhone, setWithdrawalPhone] = useState('');
  const [withdrawalOwnerName, setWithdrawalOwnerName] = useState('');
  const [withdrawalSubmitting, setWithdrawalSubmitting] = useState(false);
  const [withdrawalMessage, setWithdrawalMessage] = useState('');

  const loadProgram = async () => {
    const response = await fetch(`${getApiUrl()}/api/telegram/referral-agent/me`, { headers: telegramHeaders() });
    if (response.status === 401) {
      setState('auth-required');
      return;
    }
    const data = await response.json() as AgentProgram & { error?: string };
    if (!response.ok) throw new Error(data.error ?? 'የወኪል መረጃዎን መጫን አልተቻለም።');
    setProgram(data);
    setState('ready');
  };

  useEffect(() => { void loadProgram().catch((loadError) => { setError(loadError instanceof Error ? loadError.message : 'የወኪል መረጃዎን መጫን አልተቻለም።'); setState('error'); }); }, []);

  const apply = async () => {
    setApplying(true);
    setError('');
    try {
      const response = await fetch(`${getApiUrl()}/api/telegram/referral-agent/apply`, { method: 'POST', headers: { 'content-type': 'application/json', ...telegramHeaders() } });
      const data = await response.json() as { error?: string };
      if (!response.ok) throw new Error(data.error ?? 'ማመልከቻውን መላክ አልተቻለም።');
      await loadProgram();
    } catch (applyError) {
      setError(applyError instanceof Error ? applyError.message : 'ማመልከቻውን መላክ አልተቻለም።');
    } finally { setApplying(false); }
  };

  const copyLink = async () => {
    if (!program?.referralLink) return;
    await navigator.clipboard.writeText(program.referralLink);
    setCopied(true);
    window.setTimeout(() => setCopied(false), 1600);
  };

  const submitAgentWithdrawal = async () => {
    setWithdrawalSubmitting(true);
    setWithdrawalMessage('');
    try {
      const response = await fetch(`${getApiUrl()}/api/telegram/referral-agent/withdraw`, {
        method: 'POST',
        headers: { 'content-type': 'application/json', ...telegramHeaders() },
        body: JSON.stringify({ amount: withdrawalAmount, phone: withdrawalPhone, ownerName: withdrawalOwnerName }),
      });
      const data = await response.json() as { error?: string };
      if (!response.ok) throw new Error(data.error ?? 'የዊዝድሮው ጥያቄ መላክ አልተቻለም።');
      setWithdrawalAmount('');
      setWithdrawalPhone('');
      setWithdrawalOwnerName('');
      setWithdrawalMessage('የዊዝድሮው ጥያቄ ለአድሚን ተልኳል።');
    } catch (withdrawalError) {
      setWithdrawalMessage(withdrawalError instanceof Error ? withdrawalError.message : 'የዊዝድሮው ጥያቄ መላክ አልተቻለም።');
    } finally { setWithdrawalSubmitting(false); }
  };

  const statusCopy = program?.agent?.status === 'active' ? 'አክቲቭ' : program?.agent?.status === 'suspended' ? 'ታግዷል' : 'የአድሚን ፍቃድ በመጠባበቅ ላይ';
  return <main className="game-grain depth-stage mx-auto flex h-[100dvh] max-w-[520px] flex-col overflow-hidden overscroll-none bg-[hsl(var(--background))] text-[hsl(var(--foreground))] shadow-2xl">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-6 pt-5">
      <div className="mb-5 flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[hsl(var(--accent))]">REFERRAL PARTNER</p><h1 className="text-shimmer mt-1 text-2xl font-extrabold">🎁 የወኪል ፕሮግራም</h1></div><button type="button" onClick={() => setLocation('/')} className="depth-action rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-2 text-xs font-bold text-[hsl(var(--foreground)/.8)]">← BACK</button></div>
      {state === 'loading' && <div className="depth-surface rounded-2xl p-5 text-sm text-[hsl(var(--muted-foreground))]">የወኪል መረጃ እየተጫነ ነው...</div>}
      {state === 'auth-required' && <div className="depth-surface rounded-2xl border border-[hsl(var(--primary)/.5)] p-5 text-sm"><ShieldCheck className="mb-3 h-8 w-8 text-[hsl(var(--primary))]" /><p className="font-bold">ይህ ገጽ ከTelegram መተግበሪያ ውስጥ ብቻ ይከፈታል።</p></div>}
      {state === 'error' && <div className="depth-surface rounded-2xl border border-[hsl(var(--destructive)/.5)] p-5 text-sm"><p>{error}</p><button type="button" onClick={() => void loadProgram()} className="mt-4 rounded-xl bg-[hsl(var(--primary))] px-4 py-2 text-xs font-bold text-[hsl(var(--primary-foreground))]">RETRY</button></div>}
      {state === 'ready' && !program?.agent && <section className="depth-card rounded-2xl border border-[hsl(var(--primary)/.35)] bg-[hsl(var(--primary)/.08)] p-5"><Gift className="h-9 w-9 text-[hsl(var(--primary))]" /><h2 className="mt-4 text-lg font-extrabold">የVenom Bingo ወኪል ይሁኑ</h2><p className="mt-2 text-sm leading-6 text-[hsl(var(--muted-foreground))]">ልዩ ሊንክዎን ያጋሩ። በሊንኩ የተመዘገበ ተጠቃሚ ዲፖዚት ሲያደርግ ከዲፖዚቱ 5% ኮሚሽን ያገኛሉ። ማመልከቻዎ ከተላከ በኋላ አድሚኑ ያክቲቫል።</p>{error && <p className="mt-3 text-xs text-[hsl(var(--destructive))]">{error}</p>}<button type="button" data-testid="button-apply-agent" disabled={applying} onClick={() => void apply()} className="depth-action mt-5 w-full rounded-xl bg-[hsl(var(--primary))] px-4 py-3 text-sm font-extrabold text-[hsl(var(--primary-foreground))] disabled:opacity-60">{applying ? 'በመላክ ላይ...' : 'የወኪልነት ማመልከቻ ላክ'}</button></section>}
      {state === 'ready' && program?.agent && <div className="space-y-4"><section className="depth-card rounded-2xl border border-[hsl(var(--accent)/.35)] bg-[hsl(var(--accent)/.08)] p-4"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[hsl(var(--accent))]">ACCOUNT STATUS</p><div className="mt-2 flex items-center justify-between gap-3"><p className="text-lg font-extrabold">{statusCopy}</p><span className="rounded-full border border-[hsl(var(--accent)/.4)] px-2 py-1 font-mono text-xs font-bold text-[hsl(var(--accent))]">{program.agent.commissionRate}%</span></div><p className="mt-2 text-xs text-[hsl(var(--muted-foreground))]">የኮሚሽን ገንዘብ ወደ የወኪል ዋሌት ይገባል።</p></section>
        {program.agent.status === 'active' && <section className="depth-card rounded-2xl border border-[hsl(var(--primary)/.35)] bg-[hsl(var(--primary)/.08)] p-4"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[hsl(var(--primary))]">YOUR UNIQUE LINK</p><p className="mt-2 break-all font-mono text-xs text-[hsl(var(--foreground)/.9)]">{program.referralLink ?? `agent_${program.agent.code}`}</p><button type="button" data-testid="button-copy-agent-link" disabled={!program.referralLink} onClick={() => void copyLink()} className="depth-action mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-[hsl(var(--primary))] px-3 py-3 text-xs font-extrabold text-[hsl(var(--primary-foreground))] disabled:opacity-60"><Copy className="h-4 w-4" /> {copied ? 'ተቀድቷል' : 'ሊንኩን ቅዳ'}</button></section>}
        <section className="depth-card rounded-2xl border border-[hsl(var(--primary)/.35)] bg-[hsl(var(--primary)/.08)] p-4"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[hsl(var(--primary))]">AGENT WALLET</p><p className="mt-2 font-mono text-3xl font-extrabold text-[hsl(var(--primary))]">{program.stats?.agentWalletBalance ?? '0.00'} ETB</p><p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">የወኪል ኮሚሽን ቀሪ ሂሳብ</p></section><section className="depth-card rounded-2xl border border-[hsl(var(--accent)/.35)] bg-[hsl(var(--accent)/.08)] p-4"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[hsl(var(--accent))]">WITHDRAW AGENT WALLET</p><div className="mt-3 space-y-2"><input type="number" min="100" step="1" value={withdrawalAmount} onChange={(event) => setWithdrawalAmount(event.target.value)} placeholder="Amount (minimum 100 ETB)" className="w-full rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-3 text-sm font-bold text-[hsl(var(--foreground))] outline-none placeholder:text-[hsl(var(--muted-foreground))]" /><input type="tel" inputMode="numeric" value={withdrawalPhone} onChange={(event) => setWithdrawalPhone(event.target.value)} placeholder="Telebirr number" className="w-full rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-3 text-sm font-bold text-[hsl(var(--foreground))] outline-none placeholder:text-[hsl(var(--muted-foreground))]" /><input value={withdrawalOwnerName} onChange={(event) => setWithdrawalOwnerName(event.target.value)} placeholder="Account owner name" className="w-full rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-3 text-sm font-bold text-[hsl(var(--foreground))] outline-none placeholder:text-[hsl(var(--muted-foreground))]" /></div>{withdrawalMessage && <p className="mt-3 text-xs font-bold text-[hsl(var(--accent))]">{withdrawalMessage}</p>}<button type="button" disabled={withdrawalSubmitting || !withdrawalAmount || !withdrawalPhone || !withdrawalOwnerName} onClick={() => void submitAgentWithdrawal()} className="depth-action mt-3 w-full rounded-xl bg-[hsl(var(--accent))] px-3 py-3 text-xs font-extrabold text-[hsl(var(--accent-foreground))] disabled:cursor-not-allowed disabled:opacity-60">{withdrawalSubmitting ? 'በመላክ ላይ...' : 'ዊዝድሮው ጠይቅ'}</button></section><section className="grid grid-cols-3 gap-2"><div className="depth-card rounded-2xl p-3"><p className="text-[10px] text-[hsl(var(--muted-foreground))]">REFERRALS</p><p className="mt-2 font-mono text-xl font-extrabold text-[hsl(var(--primary))]">{program.stats?.referrals ?? 0}</p></div><div className="depth-card rounded-2xl p-3"><p className="text-[10px] text-[hsl(var(--muted-foreground))]">DEPOSITS</p><p className="mt-2 font-mono text-xl font-extrabold text-[hsl(var(--accent))]">{program.stats?.depositTotal ?? '0.00'}</p></div><div className="depth-card rounded-2xl p-3"><p className="text-[10px] text-[hsl(var(--muted-foreground))]">EARNED</p><p className="mt-2 font-mono text-xl font-extrabold text-[hsl(var(--primary))]">{program.stats?.commissionTotal ?? '0.00'}</p></div></section>
        <section><h2 className="mb-3 text-sm font-extrabold">የኮሚሽን ታሪክ</h2><div className="space-y-2">{program.commissions?.length ? program.commissions.map((commission) => <article key={commission.id} className="flex items-center justify-between rounded-2xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] p-3"><div><p className="text-xs font-bold">{commission.depositAmount} ETB ዲፖዚት</p><p className="mt-1 text-[10px] text-[hsl(var(--muted-foreground))]">{new Date(commission.createdAt).toLocaleString()}</p></div><p className="font-mono text-base font-extrabold text-[hsl(var(--accent))]">+{commission.commissionAmount} ETB</p></article>) : <p className="depth-surface rounded-2xl p-4 text-xs text-[hsl(var(--muted-foreground))]">ገና ኮሚሽን የለም።</p>}</div></section><section><h2 className="mb-3 text-sm font-extrabold">የዊዝድሮው ታሪክ</h2><div className="space-y-2">{program.withdrawals?.length ? program.withdrawals.map((withdrawal) => <article key={withdrawal.id} className="flex items-center justify-between rounded-2xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] p-3"><div><p className="text-xs font-bold">{withdrawal.amount} ETB · {withdrawal.phone}</p><p className="mt-1 text-[10px] text-[hsl(var(--muted-foreground))]">{new Date(withdrawal.createdAt).toLocaleString()} · {withdrawal.ownerName}</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-extrabold ${withdrawal.status === 'approved' ? 'bg-[hsl(var(--accent)/.2)] text-[hsl(var(--accent))]' : withdrawal.status === 'rejected' ? 'bg-[hsl(var(--destructive)/.2)] text-[hsl(var(--destructive))]' : 'bg-[hsl(var(--primary)/.2)] text-[hsl(var(--primary))]'}`}>{withdrawal.status.toUpperCase()}</span></article>) : <p className="depth-surface rounded-2xl p-4 text-xs text-[hsl(var(--muted-foreground))]">ገና የዊዝድሮው ጥያቄ የለም።</p>}</div></section></div>}
    </div>
  </main>;
}
...
type AdminAgent = { id: number; telegramId: number; code: string; status: 'pending' | 'active' | 'suspended'; commissionRate: string; activatedAt: string | null; createdAt: string; referralCount: number; commissionTotal: string; user: { firstName: string; lastName: string | null; username: string | null; phoneNumber: string; agentWalletBalance: string } | null };
...
function AdminPanel() {
  ...
  const [adminTab, setAdminTab] = useState<'overview' | 'agents' | 'users'>('overview');
  ...
  const loadRequests = async () => {
    setError('');
    const [requestResponse, settingsResponse, promosResponse, usersResponse, agentsResponse] = await Promise.all([
      ...
      fetch(`${getApiUrl()}/api/telegram/admin/referral-agents`, { headers: telegramHeaders() }),
    ]);
    if (requestResponse.status === 401 || settingsResponse.status === 401 || promosResponse.status === 401 || usersResponse.status === 401 || agentsResponse.status === 401) {
      setStatus('auth-required');
      return;
    }
    if (requestResponse.status === 403 || settingsResponse.status === 403 || promosResponse.status === 403 || usersResponse.status === 403 || agentsResponse.status === 403) {
      setStatus('denied');
      return;
    }
    const [data, settings, promoData, userData, agentData] = await Promise.all([
      ...
      agentsResponse.json() as Promise<AdminAgent[] & { error?: string }>,
    ]);
    ...
    if (!agentsResponse.ok) throw new Error(agentData.error ?? 'Agent መጫን አልተቻለም።');
  ...
  const changeAgentStatus = async (agent: AdminAgent, action: 'activate' | 'suspend') => {
    setActionKey(`agent-${agent.id}`);
    setError('');
    try {
      const response = await fetch(`${getApiUrl()}/api/telegram/admin/referral-agents/${agent.id}/${action}`, { method: 'POST', headers: { 'content-type': 'application/json', ...telegramHeaders() } });
      const data = await response.json() as { error?: string };
      if (!response.ok) throw new Error(data.error ?? 'የወኪሉን ሁኔታ መቀየር አልተቻለም።');
      await loadRequests();
    } catch (agentError) {
      setError(agentError instanceof Error ? agentError.message : 'የወኪሉን ሁኔታ መቀየር አልተቻለም።');
    } finally { setActionKey(''); }
  };
  ...
  const renderRequest = (request: AdminRequest, type: AdminRequestType) => {
    ...
    return <article key={actionKeyForRequest} className="depth-card rounded-2xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[.12em] text-[hsl(var(--muted-foreground))]">{type === 'deposit' ? '💰 DEPOSIT' : request.walletType === 'agent' ? '💸 AGENT WALLET WITHDRAWAL' : '💸 WITHDRAWAL'} #{request.id}</p>
  ...
  return <main className="game-grain depth-stage mx-auto flex h-[100dvh] max-w-[520px] flex-col overflow-hidden overscroll-none bg-[hsl(var(--background))] text-[hsl(var(--foreground))] shadow-2xl">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-6 pt-5">
      <div className="mb-5 flex items-center justify-between gap-3">
        ...
        <button type="button" data-testid="button-admin-back" onClick={() => setLocation('/')} className="depth-action rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-2 text-xs font-bold text-[hsl(var(--foreground)/.8)]">← BACK</button>
      </div>
      ...
      {status === 'ready' && requests && <div className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-2 rounded-2xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] p-1">
          ...
          <button type="button" data-testid="button-admin-agents-tab" onClick={() => setAdminTab('agents')} className={`rounded-xl px-3 py-3 text-xs font-extrabold ${adminTab === 'agents' ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : 'text-[hsl(var(--muted-foreground))]'}`}>AGENTS ({agents.length})</button>
        ...
        <div className={adminTab === 'users' ? 'hidden' : adminTab === 'agents' ? 'admin-agent-tab space-y-6' : 'space-y-6'}>
        {gameSettings && <section className="depth-card rounded-2xl border border-[hsl(var(--accent)/.35)] bg-[hsl(var(--accent)/.08)] p-4">
          ...
          <label className="mb-3 block text-xs font-bold text-[hsl(var(--foreground)/.85)]"><span>የSupport Team username</span><input type="text" value={gameSettings.supportUsername} onChange={(event) => updateSetting('supportUsername', event.target.value)} placeholder="@SupportUsername" className="mt-1 w-full rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-2.5 font-mono text-sm font-bold text-[hsl(var(--foreground))] outline-none" /></label>
          <div className="grid grid-cols-2 gap-3">
            {[['registrationBonus', 'የሬጂስተር ቦነስ', 'ETB', '0', '100000', '0.01'], ['inviteBonus', 'የኢንቫይት ቦነስ', 'ETB', '0', '100000', '0.01'], ['depositBonusPercentage', 'የዲፖዚት ቦነስ', '%', '0', '100', '0.01'], ['mainPrizePercentage', 'ዋና ጨዋታ ፕራይዝ', '%', '0', '100', '0.01'], ['leaderboardPoolPercentage', 'ሊደርቦርድ ፕራይዝ ፑል', '%', '0', '100', '0.01'], ['leaderboardFirstPercentage', 'ሊደርቦርድ 1ኛ', '%', '0', '100', '0.01'], ['leaderboardSecondPercentage', 'ሊደርቦርድ 2ኛ', '%', '0', '100', '0.01'], ['leaderboardThirdPercentage', 'ሊደርቦርድ 3ኛ', '%', '0', '100', '0.01'], ['maxCardsPerPlayer', 'ከፍተኛ የካርድ መያዣ', 'ካርድ', '1', '500', '1'], ['leaderboardCardPurchasePoints', 'ካርድ ሲገዛ ነጥብ', 'ነጥብ', '-100', '100', '1'], ['leaderboardCardReleasePoints', 'ካርድ ሲለቀቅ ነጥብ', 'ነጥብ', '-100', '100', '1'], ['leaderboardWinPoints', 'ቢንጎ ሲያሸንፍ ነጥብ', 'ነጥብ', '-100', '100', '1']].map(([field, label, suffix, min, max, step]) => <label key={field} className="block text-xs font-bold text-[hsl(var(--foreground)/.85)]">
              <span>{label}</span>
              <div className="mt-1 flex items-center rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3">
        ...
        <section className="depth-card rounded-2xl border border-[hsl(var(--primary)/.35)] bg-[hsl(var(--primary)/.08)] p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input value={promoCode} onChange={(event) => setPromoCode(event.target.value)} placeholder="CODE2026" className="rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-3 text-sm font-bold uppercase text-[hsl(var(--foreground))] outline-none placeholder:text-[hsl(var(--muted-foreground))]" />
            <input type="number" min="0.01" step="0.01" value={promoReward} onChange={(event) => setPromoReward(event.target.value)} placeholder="Reward ETB" className="rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-3 text-sm font-bold text-[hsl(var(--foreground))] outline-none placeholder:text-[hsl(var(--muted-foreground))]" />
            <input type="number" min="1" step="1" value={promoLimit} onChange={(event) => setPromoLimit(event.target.value)} placeholder="Max uses (optional)" className="rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-3 text-sm font-bold text-[hsl(var(--foreground))] outline-none placeholder:text-[hsl(var(--muted-foreground))]" />
            <input type="datetime-local" value={promoExpiresAt} onChange={(event) => setPromoExpiresAt(event.target.value)} className="rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-3 text-xs font-bold text-[hsl(var(--foreground))] outline-none" />
          </div>
          ...
          <div className="mt-4 space-y-2">{promos.length ? promos.map((promo) => <div key={promo.id} className="flex items-center justify-between gap-2 rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] p-3"><div className="min-w-0"><p className="font-mono text-sm font-extrabold text-[hsl(var(--primary))]">{promo.code}</p><p className="text-[11px] text-[hsl(var(--muted-foreground))]">{promo.rewardAmount} ETB · {promo.redemptionCount}{promo.maxRedemptions ? `/${promo.maxRedemptions}` : ''} uses</p></div><button type="button" disabled={actionKey === `promo-${promo.id}`} onClick={() => void togglePromo(promo)} className={`rounded-lg px-2.5 py-2 text-[10px] font-extrabold ${promo.isActive ? 'bg-[hsl(var(--destructive))] text-white' : 'bg-[hsl(var(--accent))] text-[hsl(var(--accent-foreground))]'}`}>{promo.isActive ? 'DEACTIVATE' : 'ACTIVATE'}</button></div>) : <p className="text-xs text-[hsl(var(--muted-foreground))]">ምንም Promo Code የለም።</p>}</div>
        </section>
        <section className="agent-section depth-card rounded-2xl border border-[hsl(var(--accent)/.35)] bg-[hsl(var(--accent)/.08)] p-4">
          <div className="mb-4"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[hsl(var(--accent))]">REFERRAL AGENTS</p><h2 className="mt-1 text-sm font-extrabold">የወኪል ማመልከቻዎች</h2></div>
          <div className="space-y-2">{agents.length ? agents.map((agent) => <article key={agent.id} className="rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] p-3"><div className="flex items-start justify-between gap-3"><div><p className="font-bold">{agent.user ? `${agent.user.firstName} ${agent.user.lastName ?? ''}` : 'Unknown user'}</p><p className="mt-1 font-mono text-[10px] text-[hsl(var(--muted-foreground))]">{agent.code} · {agent.commissionRate}%</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-extrabold ${agent.status === 'active' ? 'bg-[hsl(var(--accent)/.2)] text-[hsl(var(--accent))]' : agent.status === 'suspended' ? 'bg-[hsl(var(--destructive)/.2)] text-[hsl(var(--destructive))]' : 'bg-[hsl(var(--primary)/.2)] text-[hsl(var(--primary))]'}`}>{agent.status.toUpperCase()}</span></div><div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-[hsl(var(--muted-foreground))]"><p>Telegram ID: <span className="font-mono text-[hsl(var(--foreground)/.85)]">{agent.telegramId}</span></p><p>Phone: <span className="font-mono text-[hsl(var(--foreground)/.85)]">{agent.user?.phoneNumber ?? '—'}</span></p><p>Referrals: <span className="font-mono text-[hsl(var(--foreground)/.85)]">{agent.referralCount}</span></p><p>Commission: <span className="font-mono text-[hsl(var(--accent))]">{agent.commissionTotal} ETB</span></p><p>Agent Wallet: <span className="font-mono text-[hsl(var(--primary))]">{agent.user?.agentWalletBalance ?? '0.00'} ETB</span></p><p>Applied: <span className="text-[hsl(var(--foreground)/.85)]">{new Date(agent.createdAt).toLocaleString()}</span></p></div><p className="mt-2 text-[11px] text-[hsl(var(--muted-foreground))]">Activated: {agent.activatedAt ? new Date(agent.activatedAt).toLocaleString() : 'Not activated'}</p><button type="button" data-testid={`button-admin-agent-${agent.status === 'active' ? 'suspend' : 'activate'}-${agent.id}`} disabled={actionKey === `agent-${agent.id}`} onClick={() => void changeAgentStatus(agent, agent.status === 'active' ? 'suspend' : 'activate')} className={`mt-3 w-full rounded-lg px-3 py-2 text-[10px] font-extrabold disabled:opacity-60 ${agent.status === 'active' ? 'bg-[hsl(var(--destructive))] text-white' : 'bg-[hsl(var(--accent))] text-[hsl(var(--accent-foreground))]'}`}>{agent.status === 'active' ? '⏸ SUSPEND AGENT' : agent.status === 'pending' ? '✅ ACTIVATE AGENT' : '↻ ACTIVATE AGENT'}</button></article>) : <p className="text-xs text-[hsl(var(--muted-foreground))]">ምንም የወኪል ማመልከቻ የለም።</p>}</div>
        </section>
        <section className="depth-card rounded-2xl border border-[hsl(var(--accent)/.35)] bg-[hsl(var(--accent)/.08)] p-4">
          ...
          <label className="block text-xs font-bold">
            <span>ምስል አፕሎድ</span>
            <input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setBroadcastPhoto(event.target.files?.[0] ?? null)} className="mt-1 w-full rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-3 text-sm text-[hsl(var(--foreground))] file:mr-3 file:rounded-lg file:border-0 file:bg-[hsl(var(--primary))] file:px-3 file:py-2 file:font-bold file:text-[hsl(var(--primary-foreground))]" />
            <span className="mt-1 block text-[11px] font-normal text-[hsl(var(--muted-foreground))]">JPEG, PNG ወይም WebP፣ እስከ 5 MB</span>
          </label>
          <label className="mt-3 block text-xs font-bold">
            <span>መልዕክት</span>
            <textarea value={broadcastCaption} onChange={(event) => setBroadcastCaption(event.target.value)} maxLength={1024} rows={4} placeholder="መልዕክትዎን እዚህ ይጻፉ..." className="mt-1 w-full resize-none rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-3 text-sm text-[hsl(var(--foreground))] outline-none placeholder:text-[hsl(var(--muted-foreground))]" />
          </label>
        ...
        {adminTab === 'users' && <section className="space-y-4">
          <input value={userSearch} onChange={(event) => setUserSearch(event.target.value)} placeholder="Search name, username, phone, Telegram ID..." className="w-full rounded-xl border border-[hsl(198_60%_74%)] bg-[hsl(var(--card))] px-3 py-3 text-sm text-[hsl(var(--foreground))] outline-none placeholder:text-[hsl(var(--muted-foreground))]" />
...
function AppShell({ children, tab, setTab, gameMode = false }: { children: ReactNode; tab: Tab; setTab: (tab: Tab) => void; gameMode?: boolean }) {
  return <main className={`game-grain depth-stage relative mx-auto flex h-[100dvh] max-w-[520px] flex-col overflow-hidden overscroll-none bg-[hsl(var(--background))] text-[hsl(var(--foreground))] shadow-2xl ${gameMode ? 'game-shell' : ''}`}><Header />{children}<ScoreChangeToast /><BottomNav active={tab} onChange={setTab} /></main>;
}

function Router() {
  const [location] = useLocation();
  return <ErrorBoundary resetKey={location}><Switch><Route path="/" component={Home} /><Route path="/game" component={GamePage} /><Route path="/winner" component={GamePage} /><Route path="/agent" component={AgentDashboard} /><Route path="/admin" component={AdminPanel} /><Route component={NotFound} /></Switch></ErrorBoundary>;
}
